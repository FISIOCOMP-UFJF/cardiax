...
void LSODA::advance(double * y, double & t, double & dt)
{
  int istate = 1;
  int size = ode->get_num_state_vars();
  double told = t;
  double tout = t + dt;
  //double rtol = 1e-4;
  //double atol = 1e-6;

  //vector<double> yy(y, y + size);
  vector<double> yy;
  yy.assign(y, y+size);
  //cout << yy.size() << endl;
  ...
  //   cout << yy[i] << " ";
  //cout << endl;
  //cout << "from " << t << " to " << tout << endl;

  // ADVANCE
  //lsoda_update(size, yy, yout, &t, tout, &istate, nullptr, rtol, atol);
  lsoda_update(size, yy, yout, &t, tout, &istate, nullptr, 1e-6, 1e-6);
  ...
  // copy to right position of y vector
...
LSODA::LSODA(CellModel *model) : ODESolver(model), rtol(1e-4), atol(1e-6)
{
  cout << "ODE solver: LSODA" << endl;

  //yout = new double[model->get_num_state_vars()];
...
void LSODA::lsoda(const size_t neq, vector<double> &y,
    ...
    */
    while (1)
    {
        while (1)
        {
            ...
            correction(neq, y, &corflag, pnorm, &del, &delp, &told, &ncf, &rh, &m, _data);
    ...
    */
    ...
    if (miter == 2)
    {
        ...
        for (j = 1; j <= n; j++)
        {
            ...
            ode->equation(tn_, &y[1], &acor[1]); // _data);
 ...
 */
/* ----------------------------------------------------------------------------*/
void LSODA::lsoda_update(const size_t neq,
                        ...
                        )
{
    ...
    lsoda(neq, yout, t, tout, itask, istate, iopt, jt, iworks, rworks, _data);
}
