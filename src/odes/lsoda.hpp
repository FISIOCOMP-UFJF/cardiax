 ...
 *        License:  MIT License
 *         Source:  https://github.com/dilawar/libsoda
 *
 *         C++ version of the code is based on:
 *         https://github.com/sdwfrost/liblsoda/
 *
 *       Modified: 2020-05-05
 *         Author: Bernardo M. Rocha <bernardomartinsrocha@gmail.com>
 *
 */
...
/**
 * Note
 * In this modified code the type LSODA_ODE_SYSTEM_TYPE is not used anymore.
 * Instead the LSODA class inherits from ODESolver (Cardiax) and has to
 * implement the interface (advance method). Also, now LSODA has a pointer
 * to a CellModel*, which is used to call equation, instead of using f of type
 * LSODA_ODE_SYSTEM_TYPE.
 *
 * Changed code:
 *    (*f)(*t, &y[1], &yh_[2][1], _data);
 * which was used to call f (evaluate right-hand-side) of system of ODEs
 * to:
 *    ode->equation(*t, &y[1], &yh_[2][1]);
 */
...
public:
  ...
  void prja(const size_t neq, vector<double> &y, void *_data);

  //void lsoda(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y,
  void lsoda(const size_t neq, vector<double> &y,
  ...
  void stoda(const size_t neq, vector<double> &y, void *_data);
  ...
  //                 );
  void lsoda_update(const size_t neq,
                    vector<double> &y, std::vector<double> &yout, double *t,
                    const double tout, int *istate, void *const _data,
                    double rtol = 1e-6, double atol = 1e-6 // Tolerance
  );
...
private:
  ...
  void set_rtol(double rt) { rtol = rt; };
  void set_atol(double at) { atol = at; };
  // -------------------------------------------------------
