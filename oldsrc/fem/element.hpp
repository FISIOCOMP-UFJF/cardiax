...
public:
  ...
  Element(ElementType et, std::vector<int> pnums, 
	  arma::vec3 f, arma::vec3 t, arma::vec3 n, int indx = 1, int aha = 0)
    : eltype(et), pt_nums(pnums), index(indx), aha_num(aha),
  ...
  Element(ElementType et, std::vector<int> pnums,
	  ...
	  arma::vec3 l, arma::vec3 c, arma::vec3 r, int indx = 1, int aha = 0)
    : eltype(et), pt_nums(pnums), index(indx), aha_num(aha),
  ...
  //! Get region index 
  int get_aha_num() const {return aha_num;}
  ...
  //! Set index of element's aha 
  void set_aha_num(int i) {aha_num = i;}
